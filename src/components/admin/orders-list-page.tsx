...
export function OrdersListPage() {
  ...
  return (
    <div className={adminPageClass}>
      <AdminPageHeader
        title="Orders"
        description="Manage course purchases, fulfillment & enrollment"
        actions={
          <>
      ...
      <div className={cn("rounded-2xl border border-border bg-card p-4", adminFilterBarClass)}>
        ...
        <select value={fulfillMethod} onChange={(e) => setFulfillMethod(e.target.value as Payment["method"])} className={adminFilterSelectClass} title="Default fulfill payment method">
        ...
        {loading ? (
          ...
          items.map((o) => (
            <AdminMobileCard key={o.id}>
              ...
              <AdminMobileRow label="Payment">{o.hasPayment ? `${o.methodLabel} · ${o.paymentStatus}` : "Unpaid"}</AdminMobileRow>
              <AdminMobileRow label="Enrolled">{o.isEnrolled ? "Yes" : "No"}</AdminMobileRow>
              <AdminMobileRow label="Date">{formatDate(o.createdAt)}</AdminMobileRow>
              <AdminMobileActions>
                <button type="button" onClick={() => openDetail(o)} className="inline-flex flex-1 items-center justify-center gap-1 rounded-lg border px-3 py-2 text-xs font-semibold">
                  <Eye className="h-3.5 w-3.5" /> View
                </button>
                {(o.status === "pending" || (o.status === "completed" && !o.isEnrolled)) && (
                  <button type="button" onClick={() => fulfill(o)} className="inline-flex items-center justify-center rounded-lg border border-emerald-200 px-3 py-2 text-xs font-semibold text-emerald-600">
                    <CheckCircle2 className="h-3.5 w-3.5" />
                  </button>
                )}
                {o.status === "pending" && (
                  <button type="button" onClick={() => remove(o)} className="inline-flex items-center justify-center rounded-lg border border-red-200 px-3 py-2 text-xs font-semibold text-red-600">
