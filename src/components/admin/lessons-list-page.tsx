...
export function LessonsListPage() {
  ...
  return (
    <div className={adminPageClass}>
      <AdminPageHeader
        title="Lessons"
        description="Manage course lessons — video, text, quiz & assignment content"
        actions={
          <>
      ...
      <div className={cn("rounded-2xl border border-border bg-card p-4", adminFilterBarClass)}>
        <div className="relative min-w-0 flex-1 sm:min-w-[200px]">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search title, course, section..." className={cn(inputClass, "pl-9")} />
        </div>
        <select value={courseFilter} onChange={(e) => setCourseFilter(e.target.value)} className={adminFilterSelectClass}>
          <option value="">All Courses</option>
          {courses.map((c) => <option key={c.id} value={c.id}>{c.title}</option>)}
        </select>
        <select value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)} className={adminFilterSelectClass}>
      ...
      <AdminMobileList>
        {loading ? (
          <AdminLoadingState message="Loading lessons..." />
        ) : items.length === 0 ? (
          <AdminEmptyState message="No lessons found" />
        ) : (
          items.map((row) => {
            const Icon = typeIcon(row.lessonType);
            return (
              <AdminMobileCard key={row.id}>
                ...
                <div className="mt-2 flex flex-wrap gap-1.5">
                  <span className={cn("rounded-full px-2 py-0.5 text-[10px] font-bold uppercase", typeBadge(row.lessonType))}>{row.lessonType}</span>
                ...
                <AdminMobileRow label="Section">{row.sectionTitle}</AdminMobileRow>
                <AdminMobileRow label="Duration">{row.duration}</AdminMobileRow>
                <AdminMobileActions>
